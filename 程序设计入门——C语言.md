...
​		通过第三章及本章，我们可以知道C语言描述关系和数学语言有一点不同，2 < x < 5的式子在C中表达的是一个逻辑值0或1，正确的C语言表达式是：x > 2 && x < 5

​		**字符型常量**指的是单个字符，用一对引号及其括起来的字符来表示，每个字符都有一个唯一的次序值，即**ASCII码**。

数字字符'0'，'1'，...，'9'、大写字母'A'，'B'，...，'Z'、小写字母'a'，'b'，...，'c'，分别都是按ASCII码升序连续排列。

​		所以判断一个字符c是否为大写字母，可以使用`c>='A' && c <= 'Z'`来判断。
...
​		利用else if语句，可以描述方便地下面的分段函数，并打印不同自变量下f的值。
$$
y=f(x) = \begin{cases}
  -1 &  x < 0 \\
  0 &  x= 0 \\
...
​		else总是和最近的那个if匹配，通过加大括号，可以改变if-else对应的匹配，比如将上面的程序修改为下面这段程序，此时第二个if就没有对应的else和它匹配了。
...
​		常量表达式包括整型或字符型，也可以是常量的运算表达式，所有的常量表达式的值都不能相等。
